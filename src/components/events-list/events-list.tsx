"use client";
import { EventProps, EventVoloProps } from "@/utils/types";
...
type EventsListProps = {
    events: EventProps[];
    eventsVolo: EventVoloProps[];
};

export default function EventsList({ events, eventsVolo }: EventsListProps) {
    const getLink = (
        id: string,
        name: string,
        maxVolo: number,
        ...
        return (
            ...
            >
                <div
                    className={`font-bold text-xl block ${voloNumber >= maxVolo ? "text-green-800" : "text-pink-800"}`}
                >
                    {voloNumber}/{maxVolo}
                </div>
                <div className="block shrink grow overflow-hidden">
                    <h2 className="text-ellipsis overflow-hidden whitespace-nowrap w-full">
                        {name}
                    </h2>
